...
/// <summary>
/// AF0010 — a <c>[Journey]</c> proves a write slice, and nothing else. A
/// <c>[Journey(typeof(X), …)]</c> whose <c>X</c> is a mechanically recognized read is rejected: its only
/// consumer is <c>AF0008</c>, which tracks write slices, so the marker would otherwise be inert metadata.
///
/// Together with <c>AF0008</c> (every write slice has its happy + sad journeys) this makes the relation
...
public sealed class JourneyCoversWriteAnalyzer : DiagnosticAnalyzer
{
    ...
    private static readonly DiagnosticDescriptor Rule = new(
        ...
        description: "A [Journey] is the AF0008-tracked proof of a write slice; one pointing at a read is "
