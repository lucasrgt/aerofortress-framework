...
/// AeroFortress.Framework.Abstractions for the doctor to enforce it. It fires only on files outside <c>src/</c>, so the
...
public sealed class TestInfraPurityAnalyzer : DiagnosticAnalyzer
{
    ...
    // A file is "in src/" when some path segment is exactly "src" — the AeroFortress convention root for production
