...
/// (<c>Program.cs</c>) wire only the three named layers — <c>AddLazuli</c> / <c>AddPlatform</c> /
/// <c>AddModules</c> and the matching <c>UseLazuli</c> / <c>UsePlatform</c> / <c>MapModules</c>. Any other
...
public sealed class CompositionRootAnalyzer : DiagnosticAnalyzer
{
    ...
    private static readonly DiagnosticDescriptor Rule = new(
        ...
        description: "Program.cs (the top-level statements) wires only AddLazuli / AddPlatform / AddModules and "
                   + "the matching UseLazuli / UsePlatform / MapModules. Service registration belongs in "
    ...
    private static void AnalyzeInvocation(SyntaxNodeAnalysisContext context)
    {
        ...
        {
            if (name is not ("AddLazuli" or "AddPlatform" or "AddModules"))
                Report(context, at,
        ...
        if (name.StartsWith("Use", StringComparison.Ordinal) && name is not ("UseLazuli" or "UsePlatform"))
