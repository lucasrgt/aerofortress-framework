...
// eslint-plugin-lazuli (the LZFE rules) because the package isn't published yet; a mirror that drifts from the
...
// (line-ending-insensitive) and fails on drift. The backend half (package versions) lives in `lazuli doctor`
...
export function checkMirror({ mirror, canonical }) {
  ...
  if (fingerprint(mirror) !== fingerprint(canonical))
    return {
      status: "drifted",
      messages: [
        "framework-sync: clients/eslint-plugin-lazuli/index.cjs differs from the framework canonical — rebase the "
...
if (invokedDirectly) {
  ...
  const result = checkMirror({
    mirror: read(join(appRoot, "clients", "eslint-plugin-lazuli", "index.cjs")),
