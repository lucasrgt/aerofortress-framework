...
- `[ValueObject]`: immutable, no public ctor/setters, smart constructor returning `Result<T>`,
  always-valid by construction (LZ0013). Prefer rich VOs (Money, Email, Cpf) over primitives —
...
- `[Entity]`: private ctor (EF rehydration), private setters, intention-revealing methods
  ...
  path returns through (LZ0014). EF never sees a broken entity.
- Persisted types must be MARKED: `DbSet<T>` with unmarked T, or complex member of an entity
  unmarked → LZ0021. Marks are the only way to model state.

## Slices

- One feature = one file. Handler uses `AppDb` directly — no repository/UoW (LZ0006).
...
- Errors: `(Kind, Code, Message, Fields)` — Code is a registry constant on a `*ErrorCodes`
  class, namespaced `<module>.<reason>` (LZ0018/LZ0019). Message is a developer hint in
  English, never user copy.
- Authorization is a decision on every endpoint: `.RequireAuthorization(...)` or
  `.AllowAnonymous()` explicitly (LZ0022). If `Handle` takes `ICurrentUser`, it must read it (LZ0023).
- Files ≤ 500 LOC (LZ0007). Held `Result<T>` must be checked before `.Value`/`.Error` (LZ0025).
...
- One AppDb; modules are bounded contexts by convention. A module WRITES only its own entities
  (LZ0009); reads/joins/in-process calls are free. Reference other modules by id, never EF FK.
  Cross-module effects go through the owner's service or a job; outbox/domain events only for
  genuinely async external work.

## Testing

- Tests co-located in `src/` next to the slice (LZ0003, LZ0011), categorized `[Unit]` /
  `[Integration]` / `[E2E]`.
- `[Critical]` slices: BOTH happy and sad `[Journey(typeof(Slice), JourneyPath.X)]` (LZ0008/LZ0010),
  sad asserting failure AND no state change; journeys must assert post-conditions (LZ0020);
  write-side criticals declare concurrency (`[Timestamp] byte[]? RowVersion` or
  `[ConcurrencyCheck]`) (LZ0026).
...
routes or examples — those live in code. Citations must resolve (LZ0005).
