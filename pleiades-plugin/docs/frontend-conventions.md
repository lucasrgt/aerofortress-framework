...
(`lazuli-client.ts`) unwraps Result<T>, injects auth, maps errors. Audience filter keeps only
...
from the @lazuli/react spine). Views route all three through `<Resource>` (with retry).
