...
/// framework's <c>Lazuli.Auth</c> package, wired in one call (<c>AddJwtAccessTokens</c>). The emitted code
...
public static class AuthGenerator
{
    ...
    // (ICurrentUser, AccessTokens, RefreshCookie) is framework-owned (Lazuli.Auth), so there is nothing
    ...
    // before app.Run(). Plus UseAuthentication/UseAuthorization, which the auth routes need.
    private static void WireProgram(string root, string appName, bool tenancy)
    {
        ...
        if (unusual)
        {
            ...
            Console.WriteLine("note: Program.cs looks unusual — add builder.AddAccount(); before Build(), then "
                + "app.UseAuthentication(); app.UseAuthorization(); and AccountModule.Map(app); before app.Run();");
            return;
        }
        ...
        // The auth middleware + the module routes, just before app.Run().
        var wiring = $"app.UseAuthentication();{nl}app.UseAuthorization();{nl}{nl}AccountModule.Map(app);{nl}{nl}app.Run();";
    ...
    private static void WireApiProject(string csproj, string appLower)
    {
        ...
        if (text.Contains("Lazuli.Auth"))
            return;

        var nl = text.Contains("\r\n") ? "\r\n" : "\n";
        // Lazuli.Auth carries the JWT mechanism (and JwtBearer transitively) — the app names no JWT
        ...
        var packages = string.Join(nl, new[]
        {
            ...
            "    <PackageReference Include=\"Lazuli.Auth\" Version=\"0.1.0\" />",
        });
    ...
    private static void WireTestProject(string testDir)
    {
        ...
        if (csproj is null)
        {
            Console.WriteLine($"note: no test project at {testDir} — add a <App>.Tests project with the "
                + "Lazuli.Testing.InMemory package so the integration tests can boot the app.");
            return;
        }

        var text = File.ReadAllText(csproj);
        if (text.Contains("Lazuli.Testing.InMemory"))
            return;

        var nl = text.Contains("\r\n") ? "\r\n" : "\n";
        var reference = "    <PackageReference Include=\"Lazuli.Testing.InMemory\" Version=\"0.1.0\" />";
        // Sit it next to the existing Lazuli.Testing reference when present, else in the first group.
        if (text.Contains("Lazuli.Testing\""))
        {
            text = ReplaceFirst(text,
                "<PackageReference Include=\"Lazuli.Testing\" Version=\"0.1.0\" />",
                "<PackageReference Include=\"Lazuli.Testing\" Version=\"0.1.0\" />" + nl + reference);
        }
        else
        {
        ...
        File.WriteAllText(csproj, text);
        Console.WriteLine($"added Lazuli.Testing.InMemory to {Path.GetFileName(csproj)}");
    }
