...
/// <summary>
/// The anti-desync leg of <c>lazuli doctor</c> — the enforcement half of the <b>package-first law</b>
...
///
/// A pilot opts in by declaring where the framework checkout lives in <c>Lazuli.toml</c>:
/// <code>
/// [framework]
/// repo = "../../lazuli-net"
/// </code>
...
/// <list type="bullet">
/// <item><description><b>Package version</b> — the app's referenced <c>Lazuli*</c> package versions vs the
/// framework's current <c>Version</c> (<c>build/Lazuli.Library.props</c>). Behind = the pilot is running an
/// old doctor and silently missing rules.</description></item>
/// <item><description><b>The eslint-plugin mirror</b> — <c>clients/eslint-plugin-lazuli/index.cjs</c> hashed
...
internal static class FrameworkSync
{
    ...
    {
        var manifest = Path.Combine(root, LazuliManifest.FileName);
        ...
        if (!repoRel.Success)
            return new Outcome(false, true,
                ["framework-sync: no [framework] repo in Lazuli.toml — drift against the framework checkout is not checked (declare it on dev machines)"]);
    ...
    // The app's Lazuli* package references must match the framework's current version — behind means the
    ...
    {
        var props = Path.Combine(repo, "build", "Lazuli.Library.props");
        ...
        var stale = Directory.EnumerateFiles(root, "*.csproj", SearchOption.AllDirectories)
            ...
            .SelectMany(p => Regex.Matches(File.ReadAllText(p), @"Include=""(Lazuli[^""]*)""\s+Version=""([^""]+)""")
    ...
    {
        var mirror = Path.Combine(root, "clients", "eslint-plugin-lazuli", "index.cjs");
        ...
        if (!HashOf(mirror).SequenceEqual(HashOf(canonical)))
            messages.Add(
                "framework-sync: clients/eslint-plugin-lazuli/index.cjs differs from the framework canonical — "
