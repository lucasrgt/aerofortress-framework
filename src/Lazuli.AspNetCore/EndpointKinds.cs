...
/// <see cref="App"/>, is the dominant case (a Lazuli app is UI-first): app-facing, so it must be wired by a
...
/// the client generator filters it: a <see cref="EndpointKind.Webhook"/> / <see cref="EndpointKind.Internal"/>
...
public static class EndpointKindExtensions
{
    ...
    public const string WebhookTag = "lazuli:webhook";

    /// <summary>The OpenAPI tag an internal endpoint carries — the generator excludes it from the app client.</summary>
    public const string InternalTag = "lazuli:internal";

    /// <summary>Mark an endpoint's nature. <see cref="EndpointKind.App"/> is a no-op (the default); a webhook or
    /// internal endpoint is tagged so the client generator leaves it out of the app SDK.</summary>
    /// <example>
    /// <code>
