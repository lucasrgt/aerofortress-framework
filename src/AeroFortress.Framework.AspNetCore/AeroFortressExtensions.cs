...
/// index rather than re-deriving AeroFortress's conventions every time. <see cref="AddAeroFortress"/> registers what every
/// AeroFortress app shares — the slice-aware OpenAPI document (<see cref="OpenApiExtensions.AddAeroFortressOpenApi"/>) and
/// the enum-as-name JSON convention; <see cref="UseAeroFortress"/> serves the contract. Auth, persistence, and vendor
/// adapters stay the app's own calls — the framework ships the conventions, not the app's choices.
/// </summary>
public static class AeroFortressExtensions
{
    /// <summary>Register AeroFortress's universal conventions: the slice-aware OpenAPI document and enum-as-name JSON.</summary>
    public static IServiceCollection AddAeroFortress(this IServiceCollection services)
    {
        services.AddAeroFortressOpenApi();
    ...
    /// <summary>Serve AeroFortress's framework endpoints — the OpenAPI document at <c>/openapi/v1.json</c>, the typed
    /// contract a client generates from.</summary>
    public static WebApplication UseAeroFortress(this WebApplication app)
    {
        app.MapOpenApi();
        return app;
    }
