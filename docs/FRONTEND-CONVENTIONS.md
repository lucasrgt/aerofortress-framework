...
| `LZFE015` | **No `router.replace` inside `useEffect`** — a guard redirect must be declarative (`if (terminal) return <Redirect href={…} />`), never an effect: on web expo-router freezes the source screen, so an effect-driven `replace` re-fires every render into an infinite navigation/refetch loop. `push`/`back` on a user action stay allowed. Scoped to `*.view.tsx` | **shipped** | the pilot shipped this loop twice (Splash, then ChooseRole + 5 screens) before the rule existed |
| `LZFE016` | No orphan placeholder — `// wire later`, `TODO`/`FIXME`, `WAR-*`, or `@ts-expect-error` on a data call | planned | mirror of `LZSELF002` — "almost done" is not done (renumbered from LZFE015 when the navigation rule claimed that slot) |
