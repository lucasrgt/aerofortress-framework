...
| `LZFE016` | **Session one door** — the bearer token is written through one seam (`lib/session`), where the write is paired with the right cache move: a rotation resets the session shape (`me`), a sign-in/out wipes the WHOLE cache (the spine seam's `onSignedIn`/`onIdentityChanged` — nothing forces a sign-out before a sign-in, so the previous account's cached rows must die); a `*.viewModel`/`*.view` importing the token setter (`setAccessToken`…) directly — **or writing a token-ish key straight to storage** (`localStorage`/`AsyncStorage`/`SecureStore.setItem("…token…", …)`) — is the scattered write that forgets it | **shipped** | pauta: a forgotten reset after registration bounced the new user back to `/login`; hostpoint: a sign-in without sign-out kept a stale COMPLETE `my-traveler` that skipped the new account's onboarding straight to the map |
