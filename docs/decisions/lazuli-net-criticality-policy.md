...
the rung that forces the decision to be made (`LZ0029`).
**Lineage:** the same "a decision, never an omission" posture as `LZ0022` (every endpoint declares its
...
to write `[Critical]`, and `LZ0008`/`LZ0010` enforce the journeys only for those marked classes. That is the
...
`LZ0022` was built to close for authorization:
...
> the silent failure `LZ0022` named for the open endpoint.

Two facts make this a framework concern, not a per-app lint preference:

1. **The criticality bar is already framework-owned and enforced** (`[Critical]`, `[Journey]`, LZ0008/10).
...
| `"strict"` | an undecided slice is **treated as** `[Critical]` | unmarked slice ⇒ critical ⇒ journeys required; `[NonCritical]` is the opt-out | inert (the obligation lives in LZ0008/10) |
...
- `LZ0029` — under the `explicit` policy, a `[Slice]` declaring neither `[Critical]` nor `[NonCritical]`.
  Next free id; LZ0001–LZ0028 are taken (LZ0028 = `PageOrderTiebreakerAnalyzer`).

Existing, cited as-is and rewired (not renumbered): `LZ0008` (`CriticalJourneyAnalyzer.cs`), `LZ0010`
...
| 4 | Doctor enforcement named + severity path | 8.5 | `LZ0029` Error, implemented + tested across all three levels; LZ0008/10 strict interaction tested | severities not yet mapped across `production`/`strict`/`prototype` profiles |
| 5 | Diagnostic-ID truthfulness (rubric C8.5) | 9.5 | `LZ0029` genuinely net-new; the id-collision check (LZ0028 taken) is in the code comment | none material |
| 6 | Testability | 8.5 | LZ0029 ×3 levels, LZ0008/LZ0010 strict cases, manifest legal+typo cases — all green | the MSBuild projection itself is not exercised by the solution build (the targets import only on package consumption); verified by reasoning, not a test |
| 7 | Anti-theater honesty — does it close the hole it claims? | 8.0 | `explicit` makes "nobody decided" impossible to ship; `strict` defaults the safe direction | it cannot judge whether a `[NonCritical]` is *honest* — a lazy author can stamp `[NonCritical]` to silence LZ0029, the same Rice-ceiling every marker rule hits |
...
`LZ0022` made for authorization — but the proposal must not oversell it as proving the decision is *correct*,
