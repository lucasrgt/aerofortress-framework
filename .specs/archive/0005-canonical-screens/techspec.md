...
(`scaffold-feature.mjs`) and shaping it into the form recipe. Promote AFFE024–026 + web jsx-a11y to
...
- `examples/sample-app/frontend/core/src/itemForm/ItemForm.viewModel.ts` — create-item command:
  react-hook-form + zod schema, mutation via the generated client, error surfaced per AFFE013.
...
- `.../itemForm/itemForm.i18n.ts` — ptBR/esES/enUS, full parity (AFFE011).
...
- `frontend-sdk/eslint.config.mjs` — AFFE024–026 `warn → error`; jsx-a11y block for the sample web
...
- Form recipe shape (FINAL — what agents copy): zod schema named `<Feature>Schema` co-located in the
  ...
  submitting, submitError }` — the View stays data-layer-free (AFFE001) and the VM platform-agnostic
  (AFFE009). Field-level errors render in `Field error=`; the mutation error renders as the
...
- [ ] `mutation failure surfaces as role=alert and the button leaves loading` — AFFE013 made visible.
- [ ] `submit button disables and announces while pending` — aria-busy contract from 0003.
- [ ] `i18n parity across ptBR/esES/enUS for the new namespace` — AFFE011 green by construction.
- [ ] `Items renders kit EmptyState on empty and ErrorState with working retry on error` —
- [ ] `sample tree lints clean with AFFE024–026 and jsx-a11y at error` — the promotion proof.
...
Items module at all (Wallets only), so the recipe mirrors the REAL Deposit slice (AF0012 1:1;
